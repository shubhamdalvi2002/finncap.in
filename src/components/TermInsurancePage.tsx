...
export const TermInsurancePage: React.FC<TermInsurancePageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 py-12 px-[5%] relative font-sans">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[350px] bg-blue-100/60 rounded-full blur-3xl pointer-events-none z-0" />
      ...
      <div className="max-w-5xl mx-auto relative z-10 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center bg-white border border-slate-200 rounded-3xl p-6 sm:p-10 shadow-sm">
          <div className="md:col-span-7 space-y-5 text-left">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-bold border border-blue-100">
              <Umbrella size={16} />
              <span>Family Income Protection up to ₹5 Crores</span>
            </div>
            ...
            <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">
              Secure your family’s financial future with pure term protection plans offering high sum assured coverage at low premiums, critical illness riders, accidental death benefits, and tax exemptions under Section 80C & 10(10D).
            </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { title: 'High Cover Amount', desc: '₹50 Lakhs to ₹5 Crores+', icon: <ShieldCheck className="text-blue-600" size={20} /> },
            { title: 'Tax Benefits', desc: 'Exemptions u/s 80C & 10(10D)', icon: <Award className="text-emerald-600" size={20} /> },
            { title: 'Critical Illness', desc: 'Accelerated Payouts on Diagnosis', icon: <Building2 className="text-purple-600" size={20} /> },
            { title: '99%+ Claim Ratio', desc: 'Trusted Top Insurers in India', icon: <PhoneCall className="text-amber-600" size={20} /> }
          ].map((item, idx) => (
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-10 shadow-sm space-y-6 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-700">
            {[
              { title: 'Pure Life Cover', desc: 'Provides lump-sum financial payout to your nominees in event of unforeseen death.' },
              { title: 'Critical Illness Rider', desc: 'Lump-sum payout upon diagnosis of 30+ critical illnesses like cancer or heart attack.' },
              { title: 'Accidental Death Benefit', desc: 'Additional sum assured paid out in case of accidental death during the policy term.' },
              { title: 'Waiver of Premium Rider', desc: 'Future premiums waived off if policyholder faces permanent disability due to accident.' },
              { title: 'Return of Premium (TROP)', desc: 'Option to receive 100% of all paid premiums back upon surviving policy term maturity.' },
              { title: 'Flexible Income Options', desc: 'Choose lump-sum payout or monthly income stream for your dependent family members.' }
            ].map((cov, idx) => (
          ...
          <div className="pt-4 flex flex-wrap items-center justify-between gap-4 border-t border-slate-100">
            <div className="text-xs text-slate-500">
              <strong className="text-slate-800">Top Insurers We Compare:</strong> LIC, Max Life, HDFC Life, ICICI Prudential, SBI Life, Tata AIA
            </div>
            ...
            >
              ...
              <div className="space-y-2 text-center">
                ...
                <p className="text-xs text-slate-600">
                  Submit your request or connect your custom Google Form link below to receive instant term insurance quotes.
                </p>
              </div>

              {isSubmittedNotice ? (
                <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-2xl p-4 text-center space-y-2 text-xs">
                  <p className="font-bold">Inquiry Registered for Term Life Insurance!</p>
