...
async function startServer() {
  ...
  const fetchMarketIndicators = async () => {
    ...
    if (!apiKey) {
      ...
      try {
        // Fetching some indicators from FMP or similar
        // For now, we'll use a mix of real and simulated for specific Indian context if not available
        ...
        // Derive market return from Nifty 50 if possible, or keep default
        // For this demo, we'll keep it dynamic
  ...
  const fetchStocks = async () => {
    const apiKey = process.env.FINANCIAL_API_KEY;
    if (!apiKey) {
      console.warn('FINANCIAL_API_KEY is missing. Using simulated data.');
      ...
      try {
        // Mapping of symbols for FMP (NSE stocks suffix .NS)
        const symbols = [
